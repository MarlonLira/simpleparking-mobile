...
import Diners from '../../Images/diners.gif';



import { Description, TextDescription } from './styles';

const ListCreditCard = ({ data, onPressDelete, onPressPut }) => {
      ...
      <TouchableRipple onPress={() => getOptions()}>
        <View style={{ flexDirection: 'row' }}>
          ...
          <Animated.View style={[style.cardContent, { marginLeft: margin }]}>
            ...
            <View>
              <Description>
                <TextDescription>Cartão de {data.type == 'credit' ? 'Crédito' : 'Débito'}</TextDescription>
              </Description>

              <Description>
                <TextDescription>{data.flag.toUpperCase()} ****{data.number}</TextDescription>
              </Description>
            </View>

            <View>
              <Image source={returnImage(data.flag)} />
            </View>
